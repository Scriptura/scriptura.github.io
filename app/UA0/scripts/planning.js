...
switch (patternType) {
  ...
  case 'CUSTOM':
    const savedCustomPattern = localStorage.getItem('custom-pattern');
    if (savedCustomPattern) {
      return stringToPattern(savedCustomPattern);
    }
...
localStorage.setItem('custom-pattern', cleanPatternInput(textarea.value));
}
...
if (validatePattern(cleanedValue)) {
  ...
  localStorage.setItem('custom-pattern', cleanedValue);
} else {
  // En cas d'erreur, restaurer la dernière valeur valide
  const savedPattern = localStorage.getItem('custom-pattern');
  if (savedPattern) {
    this.value = formatPatternForDisplay(savedPattern);
  } else {
    // Si pas de pattern sauvegardé, revenir au pattern initial
...
// Chargement du pattern personnalisé ou du pattern par défaut
if (savedCustomPattern && validatePattern(savedCustomPattern)) {
  document.getElementById('pattern-custom').value = formatPatternForDisplay(savedCustomPattern);
} else {
  // Si pas de pattern sauvegardé ou pattern invalide, utiliser le pattern initial
  const initialPattern = getInitialPattern(document.getElementById('pattern-select').value);
  updatePatternTextarea(initialPattern);
}
};

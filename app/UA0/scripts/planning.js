...
const rotationPatternIDE = [
  ...
  function updatePatternTextarea(pattern) {
    ...
    textarea.value = patternToString(pattern);
    localStorage.setItem('custom-pattern', textarea.value);
  }
  ...
  document.getElementById('pattern-custom').addEventListener('blur', function() {
    if (validatePattern(this.value)) {
      ...
      if (savedPattern) {
        ...
        this.value = patternToString(initialPattern);
      }
    }
  });
  ...
  window.onload = () => {
    ...
    if (savedCustomPattern && validatePattern(savedCustomPattern)) {
      document.getElementById('pattern-custom').value = savedCustomPattern;
    } else {

...
window.addEventListener('load', () => {
  ...
  const mapsDark = (() => {
    document.querySelectorAll('.map-dark .leaflet-layer').forEach(function(map) {
      map.style.filter = 'grayscale(1) invert(1) brightness(1.1) contrast(.7)'
    })
  })()
  const mapsOld = (() => {
    document.querySelectorAll('.map-old .leaflet-layer').forEach(function(map) {
      map.style.position = 'relative'
      map.style.filter = 'sepia(1)'
    })
  })()
  const mapsOld2 = (() => {
    document.querySelectorAll('.map-old .test-map').forEach(function(map) {
      map.style.position = 'absolute'
      map.style.inset = '0'
      map.style.zIndex = '500'
      map.style.backgroundImage = 'radial-gradient(transparent, #333)'
      map.style.filter = 'url(../sprites/filter.svg#noise) sepia(1) opacity(.5)'
    })
  })()
  const mapsOld3 = (() => {
    document.querySelectorAll('.map-old .test-map2').forEach(function(map) {
      map.style.position = 'absolute'
      map.style.inset = '0'
      map.style.zIndex = '500'
      map.style.backgroundImage = 'radial-gradient(transparent, hsl(25 34% 33% / .5))'
    })
  })()
  /*
  const mapsArtist = (() => {
    document.querySelectorAll('.map-artist .leaflet-layer').forEach(function(map) {

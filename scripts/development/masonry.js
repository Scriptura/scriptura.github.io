class VGrid {
  ...
  resizeAllGridItems() {
    // @bugfix Un premier calcul peut s'avérer insufisant, causant des bugs de rendu sur la longeur des items.
...
for (const masonry of document.querySelectorAll('.masonry')) {
  ...
  window.addEventListener('load', () => {
    // Préférable dans cette configuration à "DOMContentLoaded"
  ...
  window.addEventListener('scroll', () => {
    // @note Cette astuce permet de corriger un bug dû à un conflit avec l'attribut `loading='lazy'` si présent sur des images.
    let resizeTimeout
    clearTimeout(resizeTimeout)
    resizeTimeout = setTimeout(() => {
      grid.resizeAllGridItems()

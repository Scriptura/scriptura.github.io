...
--
-- DROP préventif (idempotence) :
--   CREATE OR REPLACE VIEW ne peut pas modifier le nombre ou le type des colonnes
--   existantes. Le DROP CASCADE garantit la re-exécution propre quelle que soit
--   la version précédente de la vue. v_master_health_audit est une vue feuille —
--   aucun objet ne dépend d'elle — CASCADE est sans risque.
--
-- Pré-requis : ANALYZE exécuté sur les tables auditées.
-- ==============================================================================

DROP VIEW IF EXISTS meta.v_master_health_audit CASCADE;

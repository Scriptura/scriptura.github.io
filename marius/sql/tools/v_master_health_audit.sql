...
--   reste exposé pour diagnostic mais ne pénalise pas le score.
--
...
--     WARNING    : 4-7B          — un gap inter-colonnes subsiste (1 colonne mal placée)
--     INVESTIGATE: >= 8B         — au moins une unité MAXALIGN gaspillée ; sur une
--                                  table fixe = layout cassé ; sur une table varlena
--                                  post-ANALYZE : peut être du tail padding légitime
--                                  (à croiser avec density_drift_alert).
...
WITH component_status AS (
    SELECT
        ...
        -- padding_bytes : depuis ECSM v2.2 (Proposition 3)
        m.padding_bytes
...
SELECT
    ...
    brin_drift_alert,
    -- padding_category (v2.2 — Proposition 3 Waste Management) :
    --   Colonne informative pure — ne contribue pas au debt_score.
    --   Croiser avec density_drift_alert pour distinguer padding structurel
    --   d'un tail MAXALIGN légitime sur les tables varlena.
    CASE
